...
from data_platform_architect.config import load_config
...
def test_environment_override_is_typed() -> None:
    settings = load_config(
        DEFAULT_CONFIG,
        environment={
            "ARCHITECT__PROVIDER__MAX_RETRIES": "5",
            "ARCHITECT__TOOLS__ALLOW_MUTATIONS": "false",
        },
    )
...
def test_unsafe_approval_configuration_is_rejected() -> None:
    with pytest.raises(ValidationError, match="Mutations cannot be enabled"):
        load_config(
            DEFAULT_CONFIG,
            environment={
                "ARCHITECT__TOOLS__ALLOW_MUTATIONS": "true",
                "ARCHITECT__APPROVAL__MODE": "disabled",
            },
        )
