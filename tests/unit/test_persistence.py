...
from data_platform_architect.models import PlatformBriefInput, RunState
from data_platform_architect.persistence import JsonRunStateRepository


def state() -> RunState:
    return RunState(
        run_id="safe-run_01",
        request=PlatformBriefInput(case_id="case", title="Title", description="Description"),
    )
...
def test_json_repository_retries_transient_replace_lock(tmp_path: Path) -> None:
    ...
    with patch("data_platform_architect.persistence.os.replace", side_effect=flaky_replace):
